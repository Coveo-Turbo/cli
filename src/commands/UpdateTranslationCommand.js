...
export default class UpdateTranslationCommand extends Command {
    ...
    configure() {
        ...
        this.args.add(new InputOption('word', InputOption.string));
    }
    ...
    action() {
        ...
        try {
            this.service.update(word, translations, target, type);
        } catch(e) {
            if (Logger.DEBUG === verbosity) {
        ...
        new SuccessMessage(`The translations for word ${word} have been updated to the corresponding files: ${this.locales.map(l => `${l}.${type}`).join(', ')}`)
    }
}